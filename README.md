...
本应用的状态栏通知小图标使用 app 的彩色角色头像。状态栏是否显示彩色取决于系统：

- ColorOS / MIUI 等国产 ROM：会按小图标原图的颜色渲染，状态栏直接显示**彩色角色头像**。
- 原生 Android：系统按 alpha 把小图标染为单色剪影（系统行为，应用不可控），此时显示头像轮廓的白色剪影。

无论哪种情况，下拉通知抽屉内右侧的大图标始终为彩色角色头像。
